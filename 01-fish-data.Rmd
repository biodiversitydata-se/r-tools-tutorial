...
Follow the instructions. Your choices here would have been "in3" --> "dr10" (specifying the institution - SLU Aqua, and data resource - SERS). Your variable `fq_str` will now contain a string "data_resource_uid:dr10".
...
par(oldpar)
```
```{r gridPlot, fig.height=6, fig.width=6, include=FALSE}
palBW <- leaflet::colorNumeric(c("white", "navyblue"),
                               c(0, max(resSp@data$nSpp, na.rm = TRUE)),
                               na.color = "transparent")
oldpar <- par()
